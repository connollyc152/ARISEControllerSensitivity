...
def COMPOSITES_SIX(lat, lon, ENSO_map, NAO_map, VOL_maps, SAM_maps, BASE_map, FINAL_map, METADATA):
    ...
    axs[1,0].text(180,96, "n:  100", ha = "right", **colorbar_font)
    ...
    axs[1,1].coastlines()
    axs[1,1].text(180,112, "index [" + str(METADATA[9]) + ":" + str(METADATA[10]) + "]" , ha = "right", **colorbar_font)
    axs[1,1].text(180,96, "n: " + str(METADATA[11]), ha = "right", **colorbar_font)
    ...
    # levels_temp = np.linspace(250,290,5)
...
def COMPOSITES_SIX_colorbar(lat, lon, ENSO_map, NAO_map, VOL_maps, SAM_maps, BASE_map, FINAL_map, METADATA):
    fig, axs = plt.subplots(3, 2, figsize=(15, 10), dpi = 300, subplot_kw={'projection': ccrs.PlateCarree()})
    plt.subplots_adjust(wspace= -0.2)  
    levels_anom = np.linspace(-1,1,16) #np.linspace(-2.5,2.5,16)
    ticks_temp = [-1, -.5, 0, .5, 1]
    ...
    # cbar_ax = fig.add_axes([0.19, 0.30, 0.64, 0.03])
    cbar = fig.colorbar(cs, orientation="vertical", ticks = ticks_temp)
    cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(), **colorbar_font)
    cbar.set_label(label='Temperature Anomalies (K)', **colorbar_font)
    ...
    # cbar_ax = fig.add_axes([0.19, -0.1, 0.64, 0.03])
    cbar = fig.colorbar(cs, orientation="vertical", format='%.0f')#, ticks = ticks_temp)
    cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(), **colorbar_font)
    cbar.set_label(label='Temperature (K)', **colorbar_font)
